"""Unstructured RLS on the raw peer actions on URB (B10).

The same controller as scripts/eso.py with the observer replaced by
recursive least squares on the one-hot actions of every broadcasting peer.
No road classes, no incidence matrix, no declared operator -- which is
precisely what it is there to isolate.

    --peer-scope fleet|all   fleet (default) is the broadcast set
...
Run it through the NS launcher, so every arm faces the identical dial:
    ...
    python scripts/ns_launch.py --sigma 3 --net saint_arnoult -- \
        scripts/urls.py --id sai_urls_0 --alg-conf config1 \
...
``docs/baselines/CHECKLIST_urls.md``; the implementation is
``urb_baselines/algos/urls.py``."""
...
from urb_baselines.algos.urls import UnstructuredRLS, add_args      # noqa: E402
from urb_baselines.host import main                      # noqa: E402

if __name__ == "__main__":
    main("urls", UnstructuredRLS, extra_args=add_args, description=__doc__)
