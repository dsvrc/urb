...
#
# SEEDS: any positional arguments are taken as the torch-seed list, so one
# process can own one seed and you can inspect results between cycles:
#
#     bash scripts/sweep/sweep_sigma0.sh 0        # this process: seed 0 only
#     for s in 0 1 2 3 4; do bash scripts/sweep/sweep_sigma0.sh $s; done
#
# With no argument the same process walks every seed, still seed-major (all
# algorithms at seed 0, then all at seed 1, ...) so a complete cycle of every
# algorithm lands before any seed is repeated.
#
...
# Positional args win over TORCH_SEEDS, so one process = one seed.
if [[ $# -gt 0 ]]; then
    TORCH_SEEDS="$*"
else
...
#
# The default is generated by this script (see bootstrap below) rather than
# taken from a committed run, so the sweep is self-contained on any checkout and
# the table is a deterministic function of (net, env-seed, env-conf).
...
if [[ ! -f "$ROUTES_ABS" ]]; then
    if [[ "$ROUTES_WAS_EXPLICIT" == "1" ]]; then
        # An explicit ROUTES that does not exist is a typo, not a request to
        # generate something else. Never silently substitute a different table.
    ...
    if [[ "$DRYRUN" == "1" ]]; then
        ...
        echo "    none at $ROUTES_ABS"
        echo "    generating via pact1.py --mode dry (path generation only, no AV"
        echo "    training). Deterministic in (net=$NET, env-seed=$ENV_SEED,"
        echo "    env-conf=$ENV_CONF), so every arm of both sweeps shares one table."
        ...
        # If the checkout also carries the committed sample table, say whether
        # they agree -- results/ns_certificate was computed against that one.
        SAMPLE="$REPO_ROOT/results/sample_results/routes.csv"
        if [[ -f "$SAMPLE" ]]; then
            if cmp -s "$ROUTES_ABS" "$SAMPLE"; then
                echo "    identical to results/sample_results/routes.csv --"
                echo "    results/ns_certificate applies to this sweep unchanged."
            else
                echo "    NOTE: differs from results/sample_results/routes.csv, which is"
                echo "    what results/ns_certificate was computed against. Re-run"
                echo "    scripts/ns_certify.py --routes $ROUTES before quoting the"
                echo "    ceiling numbers alongside these runs."
            fi
        fi
        echo
    fi
